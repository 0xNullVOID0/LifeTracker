...
public static class BuienradarEndpoints
{
    public static IEndpointRouteBuilder MapBuienradarEndpoints(this IEndpointRouteBuilder routes)
    {
        routes.MapGet("/buienradar", async (BuienradarService service) =>
            ...
            }).WithName("GetAllBuienradar").WithTags("Buienradar")
            .WithSummary("Get all Buienradar station measurements from DB")
        ...
        routes.MapPost("/buienradar", async (BuienradarService service) =>
            {
                ...
                return OkOrNoContent(data);
            }).WithName("SyncBuienradar").WithTags("Buienradar")
            .WithSummary("Sync latest Buienradar station measurement").WithDescription(
                "Fetches the configured weather station(currently Heino) from their feed and stores it to DB (if the feed has been updated(timestamp is new))")
